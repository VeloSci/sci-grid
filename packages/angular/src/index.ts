...
/**
 * VeloGrid Angular Adapter
 * Enterprise-ready grid component for Angular applications.
 */
@Component({
  selector: 'velosci-grid',
  template: `<div #gridContainer style="width: 100%; height: 100%; min-height: 400px;"></div>`,
  standalone: true
})
export class VeloGridAngular implements OnInit, OnChanges, OnDestroy {
  @ViewChild('gridContainer', { static: true }) gridContainer!: ElementRef;
  
  @Input() provider!: IDataGridProvider;
  @Input() config: Partial<GridConfig> = {};

  private grid: VeloGrid | null = null;

  ngOnInit() {
    this.grid = new VeloGrid(this.gridContainer.nativeElement, this.provider, this.config);
  }
