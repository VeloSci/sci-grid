...
export class DataManager {
    ...
    public saveState(): void {
        ...
        localStorage.setItem(`velogrid_state_${this.config.persistenceKey}`, JSON.stringify(data));
    }

    public loadState(): any {
        if (!this.config.persistenceKey) return null;
        const raw = localStorage.getItem(`velogrid_state_${this.config.persistenceKey}`);
