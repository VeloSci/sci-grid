# VeloGrid Development Roadmap & Adapter Strategy

## Core Philosophy
VeloGrid is designed as a **performance-first, framework-agnostic** Data Grid engine. The core logic resides in vanilla TypeScript, interacting directly with the Canvas API and DOM for maximum efficiency. This architecture allows us to provide native-feeling experiences across any modern web framework without duplicating the complex grid logic.

---

## 1. Adapter Architecture (`./adapters`)
To support multiple frameworks, we will implement a "Thin Wrapper" strategy. The core `VeloGrid` class remains the source of truth, and each adapter handles:
1. **Lifecycle Management**: Initialization on mount, cleanup on unmount.
2. **Reactivity Mapping**: Connecting framework-specific state (Props/Signals) to `VeloGrid.updateConfig()` or `VeloGrid.updateProvider()`.
...
- [ ] **Monorepo Strategy**: Move to a workspace (e.g., PNPM or NX) where `velosci-grid/core` is a dependency for `velosci-grid/react`, etc.
...
```typescript
// adapters/react/VeloGrid.tsx
export const VeloGridComponent = ({ data, config }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const gridRef = useRef<VeloGrid | null>(null);

  useEffect(() => {
    if (containerRef.current) {
      gridRef.current = new VeloGrid(containerRef.current, data, config);
    }
