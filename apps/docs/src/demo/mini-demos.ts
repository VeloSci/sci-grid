...
import type { IDataGridProvider, ColumnHeaderInfo, GridConfig } from '@velosci-grid/core';
...
export function initMiniDemos() {
    ...
    if (typesContainer) {
        ...
        new VeloGrid(typesContainer, provider, {
            rowHeight: 35,
            headerHeight: 35,
            showRowNumbers: false,
            backgroundColor: '#0d1117',
    ...
    if (themeContainer) {
        ...
        const provider = new SimpleDemoProvider(data, headers);
        new VeloGrid(themeContainer, provider, {
            rowHeight: 40,
    ...
    if (sortingContainer) {
        ...
        const provider = new SimpleDemoProvider([...data], headers);
        const grid = new VeloGrid(sortingContainer, provider, {
            ...commonConfig,
            onSort: (col, order) => {
    ...
    if (editingContainer) {
        ...
        const provider = new SimpleDemoProvider(data, headers);
        new VeloGrid(editingContainer, provider, commonConfig);
    }
    ...
    if (headersContainer) {
        ...
        const provider = new SimpleDemoProvider(data, headers);
        new VeloGrid(headersContainer, provider, {
            ...commonConfig,
            headerHeight: 65,
    ...
    if (streamContainer) {
        ...
        const provider = new SimpleDemoProvider(data, headers);
        const grid = new VeloGrid(streamContainer, provider, commonConfig);
    ...
    if (visualizationContainer) {
        ...
        const provider = new SimpleDemoProvider(data, headers);
        new VeloGrid(visualizationContainer, provider, {
            ...commonConfig,
            headerHeight: 50,
    ...
    if (quickContainer) {
        ...
        const provider = new SimpleDemoProvider(data, headers);
        new VeloGrid(quickContainer, provider, commonConfig);
    }
}
