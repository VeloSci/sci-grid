...
VeloGrid exposes user interaction events via configuration callbacks.
...
VeloGrid detects **4 distinct zones** for right-click context menus. Each zone has its own callback.
...
Since VeloGrid renders to a standard canvas, you can also attach standard DOM listeners to the container if needed for custom behavior.
