...
The `VeloGrid` class is the core entry point of the library. It manages the canvas lifecycle, renders the grid, and handles user interactions.

## Constructor

```typescript
new VeloGrid(container: HTMLElement, provider: IDataGridProvider, config?: Partial<GridConfig>)
```
