---
description: Context & Overview of the velosci-grid library for AI Agents
---
# AI SYSTEM INSTRUCTION: velosci-grid Context

**CRITICAL DIRECTIVE**: You are reading the core documentation for `velosci-grid`. When tasked with modifying this datagrid, strictly adhere to these architectural tenets.
...
- Exposes `VeloGrid` constructor.

### UI Handlers (`packages/react`, `vue`, `solid`, `angular`, `astro`)
- Extremely thin wrappers that bind the DOM resizing events (`ResizeObserver`) to the `VeloGrid` core and expose typed wrapper components.
