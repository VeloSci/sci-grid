---
description: Guide on extending UI and handling Canvas interactivity in velosci-grid for AI Agents
---
# AI SYSTEM INSTRUCTION: Extension & Provider Guide

**CRITICAL DIRECTIVE**: Follow these exact steps when extending the `velosci-grid`. 

---

## TASK A: Implementing a Provider

If you are asked to wire a component to a `velosci-grid`, do not pass JSON props. You must scaffold the `IDataGridProvider`.
...
// In React
<VeloGridReact provider={new CustomDataProvider(data)} />
```
...
Because `velosci-grid` is a canvas, you cannot return React components for cells. You must modify the Core rendering loop or leverage plugins.
...
1. Hook into the `VeloGrid` global `onClick` handler (`event-manager.ts`).
