---
description: Architecture of velosci-grid for AI Agents
---
# AI SYSTEM INSTRUCTION: velosci-grid Architecture
...
`velosci-grid` achieves 60fps by strictly drawing only what is visible in the viewport.
...
The `VeloGrid` holds zero row data.
...
- **Editing**: Cell editing uses an overlay input. `VeloGrid` exposes `startEdit(row, col)`, which mounts an absolute-positioned HTML `<input>` directly over the canvas coordinate. Value submission calls `provider.setCellData(row, col, val)`.
