---
description: How VeloGrid handles Canvas 2D operations and interactions
---
# Canvas Rendering Architecture

`velosci-grid` relies entirely on a single `<canvas>` element. There are no DOM nodes for cells or rows.

## The Draw Loop
The engine (`VeloGrid` class) hooks into a `requestAnimationFrame` loop synced with scroll events.
It calculates:
